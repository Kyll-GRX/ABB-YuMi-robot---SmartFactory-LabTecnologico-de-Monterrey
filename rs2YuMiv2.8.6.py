...
CAM_MAX_X = 90      #mm   por el mapping de las coordenadas
CAM_MIN_X = -70 
CAM_MAX_Y = 540
CAM_MIN_Y = 270 
ROB_MAX_X = 450 
...
prev_tcp = {
    "left":  {"x": LEFT_HAND_BASE[0], "y": LEFT_HAND_BASE[1], "z": 450},
    "right": {"x": RIGHT_HAND_BASE[0], "y": RIGHT_HAND_BASE[1], "z": 450}
}
coordenadas = {"left": "" , "right": ""}
coordenadas['left']   =  f"{tcp_r['left']['z']:03},{tcp_r['left']['x']:04},{tcp_r['left']['y']:03}"
coordenadas['right']  =  f"{tcp_r['right']['z']:03},{tcp_r['right']['x']:04},{tcp_r['right']['y']:03}"
...
# Funciònes para establir una mejor escala, real, entre la camera y el robot 
...
def smooth(tcp_r, prev_tcp, label) :
    alpha = 0.25

    for axis in ["x", "y", "z"] :
        if smooth_prev[label][axis] is None :
            smooth_prev[label][axis] = tcp_r[label][axis]
        else:
            smooth_prev[label][axis] = (
                smooth_prev[label][axis]
                + alpha * (tcp_r[label][axis] - smooth_prev[label][axis])
            )
        tcp_r[label][axis] = smooth_prev[label][axis]
    return tcp_r
#EndOfFunction
...
def generate_coordinates(hand_landmarks, label, tcp_p, tcp_r, width, height):
  ...
  # Convertir coordenadas de 0-1 en pixeles
    tcp_p["x"]  =  int( tcp.x *width )
    tcp_p["y"]  =  int( tcp.y *height )
  ...
  # Se voltearon los valores para que el robot actùe como un espejo del usuario 
    ...
    tcp_r[label]["x"]  =  int(tcp.x * ESCALA_X)
    tcp_r[label]["y"]  =  int((1 - tcp.y) * ESCALA_Y)
    tcp_r[label]["z"]  =  450
  ...
  # Actualizar las coordenadas del brazo DERECHO si 'left' (left hand for right arm)
    if label == "left":
        tcp_r['left']["x"]   +=  RIGHT_HAND_BASE[0]
        tcp_r['left']["y"]   +=  RIGHT_HAND_BASE[1]
        #tcp_r['left']["z"]   +=  RIGHT_HAND_BASE[2]

    # sino actualizar las del brazo IZQUIERDO
    else: 
        tcp_r['right']["x"]  +=  LEFT_HAND_BASE[0]
        tcp_r['right']["y"]  +=  LEFT_HAND_BASE[1]
        #tcp_r['right']["z"]  +=  LEFT_HAND_BASE[2]
...
def send_coordinates(label, coordenadas, tcp_r, prev_tcp, now):
    if label == "left":
        ...
        last_time["left"]  = now
        print( f" Coordenadas TCP mano izquierda: {tcp_r['left']['x']} , {tcp_r['left']['y']} , {tcp_r['left']['z']} ")

    else:
        send_command( sock_right, coordenadas['right'] )
        last_time["right"] = now
        print( f" Coordenadas TCP mano derecha: {tcp_r['right']['x']} , {tcp_r['right']['y']} , {tcp_r['right']['z']} ")

    seen[label]           = True
    prev_tcp[label]["x"]  =  tcp_r[label]["x"]
    prev_tcp[label]["y"]  =  tcp_r[label]["y"]
    prev_tcp[label]["z"]  =  tcp_r[label]["z"]
        ...
        # Condiciòn para que SOLO se dibujen cosas CUANDO se detectan 2 manos
            if resultado.multi_hand_landmarks is not None:
                ...
                # Enviar los datos segun el brazo que corresponda a la mano detectada
                    ...
                    if( not( 
                            (prev_tcp[label]["x"] -DEAD_ZONE < tcp_r[label]["x"] < prev_tcp[label]["x"] +DEAD_ZONE) 
                             and (prev_tcp[label]["y"] -DEAD_ZONE < tcp_r[label]["y"] < prev_tcp[label]["y"] +DEAD_ZONE) 
                             and (prev_tcp[label]["z"] -DEAD_ZONE < tcp_r[label]["z"] < prev_tcp[label]["z"] +DEAD_ZONE) ) 
