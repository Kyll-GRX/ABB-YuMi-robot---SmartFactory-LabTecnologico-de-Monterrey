...
pulgar_px = None
medio_px  = None
gripper_status = {"left": 0, "right": 0}
last_grip = {"left": 0, "right": 0}
# Estado almacenado:
...
tcp_p = {"x": 0, "y": 0}
tcp_r = {
    "right": {"x": 0, "y": 0, "z": 0, "gripper": 0},
    "left": {"x": 0, "y": 0, "z": 0, "gripper": 0},
}
prev_tcp = {
    "left":  {"x": 150, "y": 350, "z": 450, "gripper": 0},
    "right": {"x": -150, "y": 350, "z": 450, "gripper": 0}
}
coordenadas = {"left": "" , "right": ""}
coordenadas['left']  = f"{tcp_r['left']['z']:03},{tcp_r['left']['x']:04},{tcp_r['left']['y']:03},{tcp_r['left']['gripper']:01}/"
coordenadas['right'] = f"{tcp_r['right']['z']:03},{tcp_r['right']['x']:04},{tcp_r['right']['y']:03},{tcp_r['right']['gripper']:01}/"
...
sent_prev = {
    "left":  {"x": 200, "y": 350, "z": 450},
    "right": {"x": -200, "y": 350, "z": 450}
}
def smooth(tcp, smooth_prev, label, alpha=0.25):
    for axis in ["x", "y", "z"]:
        if smooth_prev[label][axis] is None:
            smooth_prev[label][axis] = tcp[axis]
        else:
            smooth_prev[label][axis] = smooth_prev[label][axis] + alpha * (tcp[axis] - smooth_prev[label][axis])
        tcp[axis] = int(smooth_prev[label][axis])
    return tcp
#EndOfFunction
...
def map_range(tcp_r, label) :
    ...
    mapX = ROB_MIN_X + (tcp_r[label]["x"] - CAM_MIN_X) * (ROB_MAX_X - ROB_MIN_X) / (CAM_MAX_X - CAM_MIN_X)
    if mapX > ROB_MAX_X:
        tcp_r[label]["x"] = ROB_MAX_X
    elif mapX < ROB_MIN_X:
        tcp_r[label]["x"] = ROB_MIN_X
    else:
        tcp_r[label]["x"] = mapX

    mapY = ROB_MIN_Y + (tcp_r[label]["y"] - CAM_MIN_Y) * (ROB_MAX_Y - ROB_MIN_Y) / (CAM_MAX_Y - CAM_MIN_Y)
    if mapY > ROB_MAX_Y:
        tcp_r[label]["y"] = ROB_MAX_Y
    elif mapY < ROB_MIN_Y:
        tcp_r[label]["y"] = ROB_MIN_Y
    else:
        tcp_r[label]["y"] = mapY

    #mapZ = ROB_MIN_Z + (tcp_r[label]["z"] - CAM_MIN_Z) * (ROB_MAX_Z - ROB_MIN_Z) / (CAM_MAX_Z - CAM_MIN_Z)
    #if mapZ > ROB_MAX_Z:
    #    tcp_r[label]["z"] = ROB_MAX_Z
    #elif mapZ < ROB_MIN_Z:
    #    tcp_r[label]["z"] = ROB_MIN_Z
    #else:
    #    tcp_r[label]["z"] = mapZ
...
def generate_coordinates(hand_landmarks, label, tcp_p, tcp_r, width, height):
  ...
  # Se voltearon los valores para que el robot actùe como un espejo del usuario 
    ...
    tcp_r[label]['x']   =  int( (tcp_x) * ESCALA_X)  # le -0.5 permet de partager l'écran en 2 
    #avec l'axe des ords (0) au milieu et donc d'avoir du négatif pour la Izquierda
    tcp_r[label]['y']   =  int( (1 - tcp_y) * ESCALA_Y)
    tcp_r[label]['z']   =  450
    tcp_r               =  map_range(tcp_r, label)
    tcp_r['gripper'] = gripper_state(np, pulgar_punta, medio_punta, pulgar_px, medio_px, tcp_r['gripper'], width, height)

  # Mapear y suavizar los valores para permanecer en la zona operativa del robot
    #tcp_r  =  smooth(tcp_r, smooth_prev, label)
    
  # Crear el mensaje de cordenadas para el robot 
    coordenadas['left']  = f"{tcp_r['left']['z']:03},{tcp_r['left']['x']:04},{tcp_r['left']['y']:03},{tcp_r[label]['gripper']:01}/"
    coordenadas['right'] = f"{tcp_r['right']['z']:03},{tcp_r['right']['x']:04},{tcp_r['right']['y']:03},{tcp_r[label]['gripper']:01}/"
    return coordenadas
#EndOfFunction
...
def send_coordinates(label, coordenadas, tcp_r, now):
    if label == "left":
        send_command( sock_left, coordenadas['left'] )
        last_time["left"]  = now
        print( f" Coordenadas TCP mano izquierda: {tcp_r['left']['x']:04} , {tcp_r['left']['y']:03} , {tcp_r['left']['z']:03} , {tcp_r['right']['y']:03},{tcp_r[label]['gripper']:01} /")

    else:
        send_command( sock_right, coordenadas['right'] )
        last_time["right"] = now
        print( f" Coordenadas TCP mano derecha: {tcp_r['right']['x']:04} , {tcp_r['right']['y']:03} , {tcp_r['right']['z']:03} ,  {tcp_r['right']['y']:03},{tcp_r[label]['gripper']:01} /")

    seen[label]            = True
    sent_prev[label]["x"]  =  tcp_r["x"]
    sent_prev[label]["y"]  =  tcp_r["y"]
    sent_prev[label]["z"]  =  tcp_r["z"]
    sent_prev[label]["gripper"]  =  tcp_r["gripper"]
  ...
  # Comparar la distancia lineal normal entre las coordenadas, en pixeles, de la punta del dedo pulgar menos las del dedo medio 
    if pulgar_px is not None and medio_px is not None:
        ...
        if distancia < GRIPPER_THRESH:
            gripper_status = 0 #CERRADO
            tcp_r[label]['gripper'] = 0
        else:
            gripper_status = 1 #ABIERTO
            tcp_r[label]['gripper'] = 1
...
def send_gripper(gripper_status, previous_state, label) :
    #Si el estado es diferente, actualizar las variables y mandar el estado
    if gripper_status[label] != previous_state[label]: 
        previous_state[label] = gripper_status[label]
        #Como en el còdigo de RAPID se espera el texto "ABIERTO" o "CERRADO"
        if label == "left":
            send_command( sock_left , gripper_status[label] )
        else:
            send_command( sock_right , gripper_status[label] )
    else:
        return  
#EndOfFunction
        ...
        # Condiciòn para que SOLO se dibujen cosas CUANDO se detectan 2 manos
            if resultado.multi_hand_landmarks is not None:
                ...
                # Enviar los datos segun el brazo que corresponda a la mano detectada
                    ...
                    dx = tcp_r[label]["x"] - sent_prev[label]["x"]
                    dy = tcp_r[label]["y"] - sent_prev[label]["y"]

                    #if ( (now - last_time[label] >= SEND_PERIOD)  and  (np.hypot(dx, dy) > DEAD_ZONE  or  now - last_time[label] > DEAD_TIMER) ):
                    #    send_coordinates(label, coordenadas, tcp_r, sent_prev, now)
                    if (now - last_time[label] >= SEND_PERIOD) :
                        send_coordinates(label, coordenadas, tcp_r, sent_prev, now)  #
