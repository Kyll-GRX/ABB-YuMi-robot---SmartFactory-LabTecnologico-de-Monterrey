...
def send_coordonates(label, coordenadas, tcp_rx, tcp_ry, tcp_rz, now):
    if label == "left":
    ...
    seen[label]           = True
    prev_tcp[label]["x"]  =  tcp_rx
    prev_tcp[label]["y"]  =  tcp_ry
    prev_tcp[label]["z"]  =  tcp_rz
        ...
        if resultado.multi_hand_landmarks is not None:
            ...
            for i, hand_landmarks in enumerate(resultado.multi_hand_landmarks):
                ...
                # Crear el mensaje de cordenadas para el robot 
                    ...
                    #coordenadas = f"{tcp_rx:03}{tcp_ry:03}{tcp_rz:03}"
                ...
                # Enviar los datos segun el brazo que corresponda a la mano detectada
                    ...
                    if ( not( (prev_tcp[label]["x"] -DEAD_ZONE < tcp_rx < prev_tcp[label]["x"] +DEAD_ZONE) 
                        and (prev_tcp[label]["y"] -DEAD_ZONE < tcp_ry < prev_tcp[label]["y"] +DEAD_ZONE) 
                        and (prev_tcp[label]["z"] -DEAD_ZONE < tcp_rz < prev_tcp[label]["z"] +DEAD_ZONE) ) 
                        or (now - last_time[label] > DEAD_TIMER) ) :
                            send_coordonates(label, coordenadas, tcp_rx, tcp_ry, tcp_rz, now)
                    else: 
                        continue
                ...
                # Condiciòn para que el comando gripper solo se envìe si es diferente al estado anterior
