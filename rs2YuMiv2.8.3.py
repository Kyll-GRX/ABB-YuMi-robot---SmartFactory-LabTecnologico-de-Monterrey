...
def map_range(tcp_r, label) :
    ...
    tcp_r["x"] = ROB_MIN_X + (tcp_r["x"] - CAM_MIN_X) * (ROB_MAX_X - ROB_MIN_X) / (CAM_MAX_X - CAM_MIN_X)
    tcp_r["y"] = ROB_MIN_Y + (tcp_r["y"] - CAM_MIN_Y) * (ROB_MAX_Y - ROB_MIN_Y) / (CAM_MAX_Y - CAM_MIN_Y)
    #tcp_r["z"] = ROB_MIN_Z + (tcp_r["z"] - CAM_MIN_Z) * (ROB_MAX_Z - ROB_MIN_Z) / (CAM_MAX_Z - CAM_MIN_Z)
    tcp_r["x"] = np.clip(tcp_r["x"], ROB_MIN_X, ROB_MAX_X)
    tcp_r["y"] = np.clip(tcp_r["y"], ROB_MIN_Y, ROB_MAX_Y)
    #tcp_r["y"] = np.clip(tcp_r["y"], ROB_MIN_Z, ROB_MAX_Z)
...
def generate_coordinates(hand_landmarks, label, tcp_p, tcp_r, width, height):
  ...
  # Se voltearon los valores para que el robot actùe como un espejo del usuario 
    ...
    tcp_r["x"]  =  int( (tcp_x - 0.5) * ESCALA_X/2)  # le -0.5 permet de partager l'écran en 2 
  ...
  # Actualizar las coordenadas del brazo DERECHO si 'left' (left hand for right arm)
    if label == "left":
        tcp_r["x"]  +=  RIGHT_HAND_BASE[1]
        tcp_r["y"]  +=  RIGHT_HAND_BASE[2]
        #tcp_r["z"]  +=  RIGHT_HAND_BASE[0]

    # sino actualizar las del brazo IZQUIERDO
    else: 
        tcp_r["x"]  +=  LEFT_HAND_BASE[1]
        tcp_r["y"]  +=  LEFT_HAND_BASE[2]
        #tcp_r["z"]  +=  LEFT_HAND_BASE[0]  
  ...
  # Crear el mensaje de cordenadas para el robot 
    coordenadas = f"{tcp_r['z']:03},{tcp_r['x']:03},{tcp_r['y']:04}"
    return coordenadas
#EndOfFunction
...
def send_coordinates(label, coordenadas, tcp_r, prev_tcp, now):
    if label == "left":
        ...
        last_time["left"]  = now
        print( f" Coordenadas TCP mano izquierda: {tcp_r['x']} , {tcp_r['y']} , {tcp_r['z']} ")

    else:
        send_command( sock_right, coordenadas )
        last_time["right"] = now
        print( f" Coordenadas TCP mano derecha: {tcp_r['x']} , {tcp_r['y']} , {tcp_r['z']} ")
