...
def ERROR_get_depth_value(depth_frame, x, y, window=10):
...
def get_depth_value(depth_frame, x, y, window=10):
...
def generate_coordinates(hand_landmarks, label, tcp_p, tcp_r, width, height):
  ...
  # Convertir coordenadas de 0-1 en pixeles
    ...
    #profundidad = depth_frame.get_distance(tcp_p["x"], tcp_p["y"])

    profundidad =  get_depth_value( depth_frame, tcp_p["x"], tcp_p["y"] )
    print(f"DEPTH RAW = {profundidad:.2f}")
