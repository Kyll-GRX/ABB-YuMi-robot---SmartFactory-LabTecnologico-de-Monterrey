...
coordenadas = f"{tcp_r['z']:02},{tcp_r['x']:02},{tcp_r['y']:02}"
...
def generate_coordinates(hand_landmarks, label, tcp_p, tcp_r, width, height):
  ...
  # !NO USAR COMO COORDENADAS A ENVIAR AL ROBOT!
    ...
    tcp_y = tcp.y
    """  
    Antes, usiamos el centro triangular de la mano gracias a los dedos y muneca....
    Pero no es la mejor manera porque el landmark de la muñeca se mueve demasiado cuando giras la mano 
    Entonces, si rotas la mano, el “centro promedio” se desplaza raro y produce jitter o movimientos no coherentes
    Esto NO es el centro de la palma, MediaPipe tiene un landmark mucho más estable con MIDDLE_FINGER_MCP
  ...
  # Actualizar las coordenadas del brazo DERECHO si 'left' (left hand for right arm)
    if label == "left":
        tcp_r["x"]   =  int( (tcp_x * ESCALA_X) + RIGHT_HAND_BASE[1] )
        tcp_r["y"]   =  int( ( (1 - tcp_y) * ESCALA_Y) + RIGHT_HAND_BASE[2] )
        tcp_r["z"]   =  450

    # sino actualizar las del brazo IZQUIERDO
    else: 
        tcp_r["x"]  =  int( (tcp_x * ESCALA_X) + LEFT_HAND_BASE[0] )
        tcp_r["y"]  =  int( ( (1 - tcp_y) * ESCALA_Y) + LEFT_HAND_BASE[1] )
        tcp_r["z"]  =  450
                
  # Crear el mensaje de cordenadas para el robot 
    coordenadas = f"{tcp_r['z']:02},{tcp_r['x']:02},{tcp_r['y']:02}"
    #coordenadas = f"{tcp_r['z']:03},{tcp_r['x']:03},{tcp_r['y']:03}"    #bases diferentes entre la camara y la base del robot
    return coordenadas
#EndOfFunction
...
def send_coordinates(label, coordenadas, tcp_r, now):
    if label == "left":
        ...
        last_time["left"]  = now
        print( f" Coordenadas TCP mano izquierda: {tcp_r['z']} , {tcp_r['x']} , {tcp_r['y']} ")

    else:
        send_command( sock_right, coordenadas )
        last_time["right"] = now
        print( f" Coordenadas TCP mano derecha: {tcp_r['z']} , {tcp_r['x']} , {tcp_r['y']} ")
        ...
        # Condiciòn para que SOLO se dibujen cosas CUANDO se detectan 2 manos
            if resultado.multi_hand_landmarks is not None:
                ...
                # Enviar los datos segun el brazo que corresponda a la mano detectada
                    ...
                    if ( not( (prev_tcp[label]["x"] -DEAD_ZONE < tcp_r["x"] < prev_tcp[label]["x"] +DEAD_ZONE) 
                        ...
                        or (now - last_time[label] > DEAD_TIMER) ) :
                            send_coordinates(label, coordenadas, tcp_r, now)
                    else: 
                        continue
